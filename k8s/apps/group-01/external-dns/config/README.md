...
This directory contains files that are used to configure cert-manager.

### Directory Structure

This application contains 2 files by default:

* **cert-manager.secrets.sops.yaml** \- This is where the secrets used by cert\-manager acme solvers live\. You may not need to have this file if your chosen solver \(ie\. http\) doesn't require secrets\.
* **configmap.yaml** \- This is the base configuration file for cert\-manager\. Generally the only changes you will need to make are the acme solver you wish to use\.
...
│   └── kustomization.yaml
└── config
    ├── cert-manager.secrets.sops.yaml
    └── configmap.yaml
```
...
resources:
  - namespace.yaml
  - namespace-wildcard.yaml
  - repo-jetstack.yaml
  - helm-release.yaml
  - ../config/configmap.yaml
  - ../config/cert-manager.secrets.sops.yaml
```
...
stringData:
    SECRET_CF_API_EMAIL: certs@test.com
    SECRET_CF_API_TOKEN: abcdefghijklmnopqrstuvwxyz01234567890
```
...
data:
    # Convert secret to variable
    CONFIG_CF_API_EMAIL: ${SECRET_CF_API_EMAIL}
    ...
    values.yaml: |-
      ...
      ##
      txtOwnerId: "${CONFIG_INFRA_CLUSTER}.${CONFIG_INFRA_DOMAIN}"
