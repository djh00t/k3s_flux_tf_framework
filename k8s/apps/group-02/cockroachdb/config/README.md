...
This directory contains files that are used to configure cockroachdb.
...
* **certificate-ca.yaml** - cert-manager configuration for cockroachdb.
* **configmap.yaml** - This file contains the configuration for cockroachdb.
* **kustomization.yaml** - This file is used by kustomize to build the final YAML files.
* **zone-roach-CONFIG_INFRA_CLUSTER.CONFIG_INFRA_DOMAIN.yaml** - external-dns zone file for cockroachdb.

**Please note:** <em>If you are adding or removing files in the config directory you will need to **modify the kustomization.yaml file in the config directory** to include or exclude the files you wish to use.</em>
...
├── configmap.yaml
├── kustomization.yaml
└── zone-roach-CONFIG_INFRA_CLUSTER.CONFIG_INFRA_DOMAIN.yaml
```
...
data:
  values.yaml: |-
    ...
    conf:
      ...
      cluster-name: "${CONFIG_INFRA_CLUSTER}"
      ...
      # CockroachDB's port to listen to HTTP(s) requests.
      http-port: 8080
    ...
    ingress:
      ...
      annotations:
        ...
        # Secure mode cockroachdb uses TLS on 8080
      ...
      paths: [/]
      hosts:
        - roach.${CONFIG_INFRA_DOMAIN}
        - roach-${CONFIG_INFRA_CLUSTER}.${CONFIG_INFRA_DOMAIN}
      tls:
    ...
    init:
      ...
      provisioning:
        ...
        users:
          - name: ${CONFIG_POSTGRES_ADMIN}
            password: ${SECRET_POSTGRES_ADMIN_PASS}
            # https://www.cockroachlabs.com/docs/stable/create-user.html#parameters
            options: [CANCELQUERY,CONTROLCHANGEFEED,CONTROLJOB,CREATEDB,CREATELOGIN,CREATEROLE,LOGIN,MODIFYCLUSTERSETTING,SQLLOGIN,VIEWACTIVITY,VIEWCLUSTERSETTING,VIEWACTIVITYREDACTED]
        databases: []
        # - name:
